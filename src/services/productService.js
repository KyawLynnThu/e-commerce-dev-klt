...
const productService = {
  ...
  store: async (req) => {
    ...
    try {
      ...
      const disStartDate = dayjs(discountStartDate).format('YYYY-MM-DD')
      const disEndDate = dayjs(discountEndDate).format('YYYY-MM-DD')

      let discountTypeValue = discountType; 
      ...
      if (discountValue && parseInt(discountValue) > 0) {
        createdProductPrice = await ProductPrice.create(
          {
            productId: product.id,
            ...
            discountStartDate: disStartDate ? disStartDate : null,
            discountEndDate: disEndDate ? disEndDate : null,
          },
          { transaction: t },
        );
      } else {
      ...
      if (bulkProductquantity || bulkDiscountType || bulkDiscountValue) {
        // Check if bulkDiscountValue is a valid integer
        if (Number.isInteger(bulkDiscountValue)) {
          const createBulkPrice = await ProductBulkPrice.create(
            {
              bulkProductquantity,
              bulkDiscountType,
              bulkDiscountValue,
          ...
          if (!createBulkPrice) {
            throw new Error('Product Bulk Price creation failed.');
          }
          
        } else {
          throw new Error('bulkDiscountValue should be a valid integer.');
        }
      }
      ...
      const createdProductVar = await ProductVariant.bulkCreate(
        productVarData,
        { transaction: t },
      );
